...
from robolab.tasks.manager_based.parkour.mdp.symmetry import f1
...
@configclass
class F1ParkourAmpRunnerCfg(RslRlOnPolicyRunnerCfg):
    class_name = "AMPRunner"
    num_steps_per_env = 24
    max_iterations = 30000
    save_interval = 500
    experiment_name = "f1_parkour"
    wandb_project = "f1_parkour"
    obs_groups = {
        "policy": ["policy"],
        "critic": ["critic"],
        "discriminator": ["disc"],
    ...
    algorithm = RslRlPpoAmpAlgorithmCfg(
        ...
        symmetry_cfg=RslRlSymmetryCfg(
            ...
            data_augmentation_func=f1.compute_symmetric_states,
        ),
        amp_cfg=RslRlAmpCfg(
